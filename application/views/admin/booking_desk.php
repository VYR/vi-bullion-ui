...
$(document).ready(function(){	
	...
	$("#submit_form").on("submit", function(e){
				...
				$.post({
					type: "post",
					url:"<?php echo base_url(); ?>admin/booking_desk_ajax",
					data:data,
					processData: false,
					contentType: false,
					...
					{		
						...
						if(jsondata['status']==1)
						{
							...
							setTimeout(function(){ window.location = "<?php echo base_url(); ?>admin/booking_desk"; }, 1000);
						}
						else
						{
  ...
  <div class="content-wrapper">
	...
	<form role="form" id="submit_form" method="post" enctype="multipart/form-data">
		<div class="row" >
			...
			<div class="col-md-12 form-group">
				<label for="cat_slno" class="control-label">H2 tag <span class="text-danger">*</span></label>
				<input type="text" class="form-control" id="h2_tag2" name="h2_tag2" placeholder="Enter H2 tag" value="<?php if(!empty($record)){ echo $record['h2_tag2']; } ?>" required>
			</div>
			<div class="col-md-12 form-group">
				<label for="cat_slno" class="control-label">Pdf <span class="text-danger"></span></label>
				<?php if(!empty($record)){ ?><br>
				<a target="_blank" href="<?php echo base_url(); ?>assets/images/booking/<?php echo $record['pdf']; ?>">Click Here</a>
				<?php } ?>
