...
$(document).ready(function(){	
	...
	$("#submit_form").on("submit", function(e){
				...
				$.post({
					type: "post",
					url:"<?php echo base_url(); ?>admin/visave_promoters_ajax",
					data:data,
					processData: false,
					contentType: false,
					...
					{		
						...
						if(jsondata['status']==1)
						{
							...
							setTimeout(function(){ window.location = "<?php echo base_url(); ?>admin/visave_promoters?id="+employee_id; }, 1000);
						}
						else
						{
	...
	<form role="form" id="submit_form" method="post" enctype="multipart/form-data">
		<div class="row" >
			<div class="col-md-12 form-group">
				<label for="name">Employee<span class="text-danger">*</span></label>
			...
			<div class="col-md-12 form-group">
			<label for="cat_slno" class="control-label">Commission Percentage <span class="text-danger">*</span></label>
			<input type="text" class="form-control" id="commission" name="commission" placeholder="Enter Commission Percentage" value="<?php if(!empty($record)){ echo $record['commission']; } ?>" maxlength="5" oninput="this.value = this.value.replace(/[^0-9]/g, '')" autocomplete="off" required>
			</div>
			<div class="col-md-4 form-group">
				<label for="upload_image">&nbsp;</label><br>
				<a href="<?php echo base_url(); ?>admin/visave_promoters" class="btn btn-danger">Cancel</a>
