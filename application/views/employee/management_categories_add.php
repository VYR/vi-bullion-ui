...
$(document).ready(function(){	
	...
	$("#submit_form").on("submit", function(e){
				...
				$.post({
					type: "post",
					url:"<?php echo base_url(); ?>admin/management_categories_ajax",
					data:data,
					processData: false,
					contentType: false,
					...
					{		
						...
						if(jsondata['status']==1)
						{
							...
							setTimeout(function(){ window.location = "<?php echo base_url(); ?>admin/management_categories"; }, 1000);
						}
						else
						{
  ...
  <div class="content-wrapper">
	...
	<form role="form" id="submit_form" method="post" enctype="multipart/form-data">
		<div class="row" >
			...
			<div class="col-md-6 form-group">
			<label for="cat_slno" class="control-label">Position <span class="text-danger">*</span></label>
			<input type="text" class="form-control" id="position" name="position" placeholder="Enter position" value="<?php if(!empty($record)){ echo $record['position']; } ?>" required>
			</div>
			<div class="col-md-6 form-group">
			<label for="cat_slno" class="control-label">Display count <span class="text-danger">*</span></label>
			<select class="form-control" id="display_count" name="display_count" required>
				<option value="">Select</option>
				<option value="1" <?php if(!empty($record) && $record['display_count']==1){ echo 'selected'; } ?>>1</option>
				<option value="2" <?php if(!empty($record) && $record['display_count']==2){ echo 'selected'; } ?>>2</option>
				<option value="3" <?php if(!empty($record) && $record['display_count']==3){ echo 'selected'; } ?>>3</option>
				<option value="4" <?php if(!empty($record) && $record['display_count']==4){ echo 'selected'; } ?>>4</option>
			</select>
			</div>
			...
			<div class="col-md-4 form-group">
				...
				<a href="<?php echo base_url(); ?>admin/management_categories" class="btn btn-danger">Cancel</a>
