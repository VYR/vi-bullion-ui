	...
	<div class="box-body">
		...
		<div class="col-md-6 col-xs-4 text-right">
			<a href="<?php echo base_url(); ?>admin/vi_super_payments?super_id=<?php echo $super['id']; ?>" class="btn btn-danger">Back</a>
		</div>
		</div>
	</div>
	<!-- /.box-body -->
	...
	<input type="hidden" id="employee_id" name="employee_id" value="<?php echo $super['employee_id']; ?>">
	<input type="hidden" id="super_id" name="super_id" value="<?php echo $super['id']; ?>">
	...
	<div class="box" id="payments_div" <?php if(empty($record)){ ?>style="display:none;"<?php } ?>>
		...
		<div class="box-body table-responsive" id="">
			...
			<td>
			<input type="text" class="form-control" id="fixed_amount_<?php echo $i; ?>" name="fixed_amount[]" value="<?php if(!empty($row)){ echo $row['fixed_amount']; }else{ echo $super['amount']; } ?>" readonly>							
			</td>
			<td>
	...
	function calcAmount(id){
		...
		if(commission!=''){
			var amount=Math.round(parseInt(total_grams)*parseInt(commission));
			$("#amount_"+id).val(amount);
			if(amount!=''){
				var super_amount=Math.round((amount/10)*8);
				$("#super_amount_"+id).val(super_amount);
				var pro_amount=Math.round((amount/10)*2);
	...
	$(document).ready(function(){	
		$("#submit_form").on("submit", function(e){
			...
			if(date==''){	
				...
				$.post({
					type: "post",
					url:"<?php echo base_url(); ?>admin/vi_super_payments_ajax",
					data:data,
					processData: false,
					contentType: false,
					...
					{		
						...
						if(jsondata['status']==1)
						{
							...
							setTimeout(function(){ window.location = "<?php echo base_url(); ?>admin/vi_super_payments?super_id=<?php echo $super['id']; ?>"; }, 1000);
						}
						else
						{
