	...
	<form role="form" id="submit_form" method="post" enctype="multipart/form-data">
		<div class="row" >
			...
			<div class="col-md-3 form-group">
				...
				<input type="text" class="form-control" id="promoter_id" name="promoter_id" placeholder="Enter Promoter ID" onkeyup="get_minted_promoter()" autocomplete="off" required>
			</div>
			<div class="col-md-3 form-group">
			...
			<a href="<?php echo site_url(); ?>login" class="color1">Login Now?</a>
			</div>-->
		</div>
	</form>
</div>
...
<script>
	function get_minted_promoter(){
		$('#promoter_name').val('');	
		var promoter_id=$("#promoter_id").val();
		$.ajax({    
			type: "POST",    
			dataType: "html",    
			url: "<?php echo base_url(); ?>get_minted_promoter",    
	...
	$(document).ready(function(){			
		$("#submit_form").on("submit", function(e){
			...
			if(promoter_name==''){
				...
				$.post({
					type: "post",
					url:"<?php echo base_url(); ?>minted_register_ajax",
					data:data,
					processData: false,
					contentType: false,
